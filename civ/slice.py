...
CREW = [
    dict(id="AGT-000001", name="Scout", role="Repository Scout", tier="reader",
         ...
         tools=["fs.read"],
         permissions=[{"cap": "READ_REPO", "scope": {"path_prefix": REPO_ROOT},
                       "rate": {"per_lease": 20, "per_hour": 200}}],
         memory_scope=["self"],
    ...
    dict(id="AGT-000003", name="Verifier", role="Independent Verifier", tier="actor",
         ...
         tools=["proc.run"],
         permissions=[{"cap": "EXECUTE_SANDBOX",
...
def run_slice(con, prov, verbose=True):
    ...
    out["owner_signal_id"] = store.signal(con, "MEDIUM" if verdict_ok else "HIGH",
