...
| `/monitor` | **All-day monitoring** — opt-in camera + mic session that watches your **posture** (a frame every 1–5 min analyzed by Claude vision) and your **voice tone** (analyzed locally on-device with WebAudio — only numbers like speaking time/volume/pitch leave the browser, never audio). Live nudges while you work, a timeline of the day, and a full end-of-day analysis with habits and recommendations. Uses the Screen Wake Lock API for long phone sessions. |
